                    ...
                    foreach ($listResource[$key] as $filePath) {
                        ...
                        if (!file_exists($sourcePath)) {
                            ...
                            if (!is_dir($destPath)) {
                                ...
                                foreach ($list as $partPath) {
                                    ...
                                    if ((!empty($partPath)) && (!is_dir($fullPath))) {
                                        // only make the dir, if it not exist
                                        if ((!mkdir($fullPath, CLEANER_DEFAULT_MOD_DIR, true)) &&
                                            (!is_dir($fullPath))
                                        ) {
                                            die('The creation of the folder `' . $startFolder . $fullPath . '` failed. A file with the same name exists.');
                                        }
                                    }
                                }
                            }
                            ...
                            if (!rename($sourcePath, $destPathFile)) {
                                myEcho('WARNING-WARNING: There file `' . $sourcePath . '` could not renamed to `' . $destPathFile .
